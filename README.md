...
| **The two systems differ structurally** | The projectile flow map is **affine** (linear regression recovers it to 10⁻¹⁴ m); the orbital one contains 1/r³ and lies inside no model's hypothesis space |
...
| The literature review (24 verified sources) | [`docs/LITERATURE_REVIEW.md`](docs/LITERATURE_REVIEW.md) |
...
orbits, with the RK4 reference's own error as a dotted line ten orders of magnitude below.
...
draws — so a fresh run on the same machine reproduces the committed CSVs exactly.

**No figure in this repository was drawn or adjusted by hand.** Every one is generated by a script
from a CSV that is also committed, so a figure and its underlying numbers cannot drift apart.
...
The reference error across the whole 5-year horizon is ≈ 3 × 10⁻¹¹ AU. The smallest ML error
measured anywhere is 6.4 × 10⁻⁵ AU — six orders of magnitude larger — so the reference can be
...
2. **Not** that any model discovered a physical law. Every model was trained on data generated
   *from* the law, and the two that reproduce the physics exactly are the two that were *handed*
   it — the opposite of discovery.
...
All 24 literature citations were checked against publisher records. Nothing in
